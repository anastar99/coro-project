...
export default function HomeScreen() {
  ...
  useEffect(() => {
    async function getAllSongs() {
      try {
        ...
        console.log("Data", data, data.NextLastID);
      } catch (error) {
        console.log(error);
      }
    }
  ...
  async function getMoreSongs() {
    const resp = await fetch(
      `http://localhost:8080/songs?last_id=${nextLastID}`,
    );

    const data = await resp.json();

    console.log("more songs data", data);
    setSongs((prevSongs) => [...prevSongs, ...data.songs]);

    // add the songs to the current list
    setNextLastID(data.NextLastID);
  }
    ...
    <>
      ...
      <View style={styles.container}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <SongCard song={item} onPress={handleDeleteSong} />
          )}
          ListEmptyComponent={
        ...
        {/* TODO: should only display the More Songs button if NextLastID != null */}
        <View>
          <TouchableOpacity onPress={() => getMoreSongs()}>
            <Text>More songs</Text>
          </TouchableOpacity>
        </View>
      </View>
    </>
